...
function bootPanel() {
  ...
  function renderAccounts() {
    ...
    for (const account of state.accounts) {
      ...
      const windowText = (item) => item ? `${item.remaining_percent ?? '--'}% · ${formatDate(item.reset_at)}` : '--';
  ...
  function applySchedule(schedule) {
    ...
    state.scheduledKeys = new Set(schedule.scheduled_account_keys || []); renderPeriods('#work-periods', schedule.work_periods || []); renderPeriods('#blackout-periods', schedule.blackout_periods || []);
