...
> CLIProxyAPI 原生插件 · Linux `amd64` / `arm64` · 当前版本 **v0.0.14**
...
以下文件名已对应 `v0.0.14` Release。下载后可校验 SHA-256；浏览器验收需要
Chromium，源码构建需要 Go 1.24、Node.js 和 C 编译器，交叉构建 arm64
还需要 `aarch64-linux-gnu-gcc`。

The current release is `v0.0.14`; scripts receive and store `0.0.14` without the
leading `v`. The GitHub repository is
<https://github.com/tapaixx/codex-window-reset>. A release contains these
seven named assets:

```text
codex-window-reset_0.0.14_linux_amd64.zip
codex-window-reset_0.0.14_linux_arm64.zip
checksums.txt
...
unzip -Z1 codex-window-reset_0.0.14_linux_amd64.zip \
  | diff -u - <(printf 'codex-window-reset.so\n')
unzip -Z1 codex-window-reset_0.0.14_linux_arm64.zip \
  | diff -u - <(printf 'codex-window-reset.so\n')
```
