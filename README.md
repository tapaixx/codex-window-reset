...
The current release is `v0.0.8`; scripts receive and store `0.0.8` without the
leading `v`. The GitHub repository is
<https://github.com/tapaixx/codex-window-reset>. A release contains these
seven named assets:

```text
codex-window-reset_0.0.8_linux_amd64.zip
codex-window-reset_0.0.8_linux_arm64.zip
checksums.txt
...
unzip -Z1 codex-window-reset_0.0.8_linux_amd64.zip \
  | diff -u - <(printf 'codex-window-reset.so\n')
unzip -Z1 codex-window-reset_0.0.8_linux_arm64.zip \
  | diff -u - <(printf 'codex-window-reset.so\n')
```
